...
class RandomNoise():
    ...
    def smoothNoise2d(self, bit_depth=255, smoothing_passes=15, upper_value_limit=1):
        ...
        if self.extra < smoothing_passes:
            print("Warning, to maintain intial grid, call the RandomNoise object with a larger 'extra' value; i.e. RandomNoise(extra=your_extra)")
    ...
    for x in range(0, len(colours)):
        for y in range(0, len(colours[x])):
            ...
            if LAND:
                ...
                final = '#%02X%02X%02X' % (col, col, col)
            canvas.create_rectangle(x*sq,y*sq,(x*sq)+sq, (y*sq)+sq, fill=final, width=0, tags=("RCT"+str(p)))
